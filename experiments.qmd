    ...
    results <- results |>
      ...
      summarise(
        across(c(radius, scaled_radius, coreset_radius, 
                  time_s, coreset_time_s, streaming_memory_bytes
              ), mean),
...
baseline <- load_data() |>
  ...
  filter(dataset %in% c('hmda', 'census1990', 'athelete')) |>
  group_by(dataset) |>
...
load_data(mr=T) |>
  ...
  geom_point(data=~filter(., tau %in% taus)) +
  geom_line(data=~filter(., tau %in% taus)) +
  geom_point(data=~filter(., algorithm == 'Bera-et-al-MR')) +
  geom_line(data=~filter(., algorithm == 'Bera-et-al-MR')) +
  facet_wrap(vars(dataset), scales='free_y') +
  scale_algorithm() +
  theme_paper()
ggsave("figs/scalability-radius.png", dpi=300, width=4, height=2)
```
...
load_data(mr=T) |>
  filter(k==32) |>
  filter(parallelism %in% c(2,4,8,16)) |>
  ggplot(aes(parallelism, time_s, group=tau, color=algorithm)) +
  geom_point(data=~filter(., tau %in% taus)) +
  geom_line(data=~filter(., tau %in% taus)) +
  geom_point(data=~filter(., algorithm == 'Bera-et-al-MR')) +
  geom_line(data=~filter(., algorithm == 'Bera-et-al-MR')) +
  # geom_hline(aes(yintercept=scaled_radius, color=algorithm), data=baseline) +
  # geom_text_repel(
  #   aes(label = tau),
  #   hjust=0,
  #   nudge_x=.1,
  #   show.legend=FALSE,
  #   direction="y",
  #   data = ~filter(., parallelism == 16, algorithm == "coreset-MR", tau %in% taus)
  # ) +
  facet_wrap(vars(dataset), scales='free_y') +
  scale_algorithm() +
  theme_paper()
ggsave("figs/scalability.png", dpi=300, width=4, height=2)
```

## Streaming


```{r streaming-mem-radius-tradeoff}
baseline <- load_data() |>
  filter(k == 32, algorithm %in% c("KFC", "coreset")) |>
...
load_data(streaming = TRUE, do_summarise=FALSE) |>
  ...
  arrange(param) |>
  print() |>
  ggplot(aes(streaming_memory_bytes, scaled_radius, color=algorithm)) +
  # geom_point() +
  geom_path(stat='summary') +
  geom_pointrange(stat='summary') +
  geom_hline(aes(yintercept=scaled_radius), linetype="dashed", data=baseline) +
  geom_hline(yintercept=1) +
  scale_algorithm() +
  labs(x = "memory (bytes)", y = "scaled radius") +
  scale_x_log10(labels=scales::number_bytes) +
  facet_wrap(vars(dataset), scale='free_x')
ggsave("figs/streaming-mem-radius.png", dpi=300, width=6, height=3)
```
...
load_data(streaming = TRUE, do_summarise=FALSE) |>
  ...
  arrange(param) |>
  print() |>
  ggplot(aes(streaming_memory_bytes, time_s, color=algorithm)) +
  # geom_point() +
  geom_path(stat='summary') +
  geom_pointrange(stat='summary') +
