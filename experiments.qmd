...
# scale_shape_algorithm <- function() {
#   scale_color_manual(values = c(
#     "coreset" = 21,
#     "coreset-MR" = 21,
#     "coreset-stream" = 21,
...
p_radius <- load_data() |>
  ...
  filter(dataset %in% selected_datasets) |>
  compact_plot(k, radius, ylab="radius", xlab="k", ylog=F)
# ggsave("figs/radius-selected.pdf", width=6, height=2.5)
p_time <- load_data() |>
  filter(k >= 4) |>
  filter(( tau == 32 ) | is.na(tau)) |>
  filter(dataset %in% selected_datasets) |>
  compact_plot(k, time_s, ylab="time (s)", xlab="k", ylog=T, flabels=scales::label_number(), scales_val="fixed")
...
load_data(mr=T, do_summarise=F) |>
  ...
  scale_algorithm() +
  theme_paper()
ggsave("figs/mapreduce-radius.png", dpi=300, width=6, height=3)
```
...
load_data(mr=T) |>
  ...
  scale_algorithm() +
  theme_paper()
ggsave("figs/scalability.png", dpi=300, width=6, height=3)
```
...
load_data(streaming = TRUE, do_summarise=FALSE) |>
  ...
  arrange(param) |>
  print() |>
  ggplot(aes(streaming_memory_bytes, scaled_radius, color=algorithm, shape=algorithm)) +
  # geom_point() +
