...
load_data <- function(delta_val = 0.01) {
  con <- DBI::dbConnect(RSQLite::SQLite(), "results.db")
...
load_data() |>
  ...
  scale_x_continuous(trans="log2") +
  scale_algorithm() +
  facet_wrap(vars(dataset), scales="free_y", ncol=3) +
  labs(
    x = "coreset size",
    y = "radius (scaled by the unfair clustering radius)"
  ) +
  theme_paper()
ggsave("figs/tau-radius-k32.png", dpi=300, width=8, height=8)
```
...
load_data(delta_val=0.01) |>
  ...
  theme_paper()
ggsave("figs/tau-time-k32.png", dpi=300, width=8, height=8)
```
