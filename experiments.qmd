...
load_data <- function(delta_val = 0.01, mr = FALSE, streaming = FALSE) {
  con <- DBI::dbConnect(RSQLite::SQLite(), "results.db")
...
```{r}
kfc <- load_data() |>
  filter(algorithm == 'KFC', k == 32)

load_data(streaming = TRUE) |>
  ...
  arrange(param) |>
  print() |>
  ggplot(aes(bytes_per_point, radius, color=algorithm)) +
  geom_path() +
  geom_point() +
  # geom_hline(aes(yintercept=scaled_radius, color=algorithm), data=kfc) +
  scale_algorithm() +
