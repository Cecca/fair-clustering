...
path_to_cplex = "/home/matteo/opt/cplex/cplex/bin/x86-64_linux/cplex"
    ...
    for j in range(len(clusters)):
        ...
        for a in range(l):
            ...
            if relevant > alpha[a]*clust_size:
                max_additive_violation = max(
                    max_additive_violation, ceil(relevant - alpha[a]*clust_size))
            elif relevant < beta[a]*clust_size:
                max_additive_violation = max(
                    max_additive_violation, ceil(beta[a]*clust_size - relevant))
